...
```bash
typescript-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
typescript-app$ sam local start-api
typescript-app$ curl http://localhost:3000/
```
...
```bash
typescript-app$ sam logs -n HelloWorldFunction --stack-name typescript-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name typescript-app
```
